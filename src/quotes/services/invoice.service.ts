...
import { CounterType, QuoteStatus } from "src/app-constants/enums";
...
export class InvoiceService extends MongooseQueryService<InvoiceEntity> {
  ...
  async generateInvoice(args) {
    ...
    if (!isRevised) {
      ...
      if (!created) throw new InternalServerErrorException('Error in creating invoice');
    }
    else {

      const [invoice] = await this.query({ filter: { quotationNo: quotationNo }, paging: { limit: 1 }, sorting: [{ field: 'id', direction: SortDirection.DESC }] })
