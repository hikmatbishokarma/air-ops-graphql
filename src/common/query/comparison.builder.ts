...
import { getSchemaKey } from '../helper';
import {
  CommonFieldComparisonBetweenType,
  FilterComparisonOperators,
} from 'src/interfaces/filter-field-comparison.interface';
...
export class ComparisonBuilder<Entity extends Document> {
  ...
  build<F extends keyof Entity>(
    ...
    if (this.comparisonMap[normalizedCmp]) {
      ...
      querySelector = {
        [this.comparisonMap[normalizedCmp]]: this.convertQueryValue(
          field,
          val as Entity[F],
        ),
  ...
  private betweenComparison<F extends keyof Entity>(
    ...
    if (!this.isBetweenVal(val)) {
      throw new Error(
        `Invalid value for ${cmp} expected {lower: val, upper: val} got ${JSON.stringify(val)}`,
      );
    }
    if (cmp === 'notbetween') {
      return {
        $lt: this.convertQueryValue(field, val.lower),
        $gt: this.convertQueryValue(field, val.upper),
      };
    }
    return {
      $gte: this.convertQueryValue(field, val.lower),
      $lte: this.convertQueryValue(field, val.upper),
    };
  }
  ...
  private convertQueryValue<F extends keyof Entity>(
    field: F,
    val: Entity[F],
  ): Entity[F] {
    ...
    if (!schemaType) {
      throw new BadRequestException(
        `unknown comparison field ${String(field)}`,
      );
    }
