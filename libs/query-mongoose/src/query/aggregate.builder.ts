import { AggregateQuery, AggregateResponse } from '@app/core';
import { BadRequestException } from '@nestjs/common';
import { Document } from 'mongoose';
import { getSchemaKey, toCamelCase } from './helpers';
...
export class AggregateBuilder<Entity extends Document> {
  ...
  static convertToAggregateResponse<Entity>(
    ...
    return aggregates.map(({ _id, ...response }) => {
      return {
        ...this.extractResponse(_id as Record<string, unknown>),
  ...
  // eslint-disable-next-line @typescript-eslint/no-shadow
  private static extractResponse<Entity>(
