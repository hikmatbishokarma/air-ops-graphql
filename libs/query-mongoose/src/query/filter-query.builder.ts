...
export class FilterQueryBuilder<Entity extends Document> {
  ...
  buildQuery({
    ...
    paging,
    sorting,
  }: Query<Entity>): MongooseQuery<Entity> {
    return {
  ...
  private buildAggregateSorting(
    aggregate: AggregateQuery<Entity>,
  ): MongooseSort | undefined {
    const aggregateGroupBy = this.aggregateBuilder.getGroupBySelects(
      aggregate.groupBy,
    );
    if (!aggregateGroupBy) {
      return undefined;
    ...
    return aggregateGroupBy.reduce((sort: MongooseSort, sortField) => {
      return {
        ...sort,
        [getSchemaKey(sortField)]: MONGOOSE_SORT_DIRECTION[SortDirection.ASC],
      };
    }, {});
  }
}
