...
export abstract class ReferenceQueryService<Entity extends Document> {
  ...
  > {
    ...
    if (Array.isArray(dto)) {
      return dto.reduce(async (mapPromise, entity) => {
        ...
        const refs = await this.aggregateRelations(
          RelationClass,
          relationName,
          entity,
          filter,
    ...
    const refFilter = this.getReferenceFilter(
      relationName,
      dto,
      assembler.convertQuery({ filter }).filter,
    );
    if (!refFilter) {
      return [];
    }
    const { filterQuery, aggregate, options } =
      referenceQueryBuilder.buildAggregateQuery(
    ...
    const aggResult = (await relationModel
      .aggregate<Record<string, unknown>>(aggPipeline)
  ...
  async countRelations<Relation extends Document>(
    ...
    if (Array.isArray(dto)) {
      return dto.reduce(async (mapPromise, entity) => {
        const map = await mapPromise;
        const refs = await this.countRelations(
          RelationClass,
          relationName,
          entity,
          filter,
    ...
    return relationModel
      .countDocuments(referenceQueryBuilder.buildFilterQuery(refFilter))
      .exec();
  }
  ...
  async findRelation<Relation>(
    ...
    if (Array.isArray(dto)) {
      return dto.reduce(async (prev, curr) => {
        ...
        const ref = await this.findRelation(
          RelationClass,
          relationName,
          curr,
          opts,
    ...
    const assembler = AssemblerFactory.getAssembler(RelationClass, Document);
    const filterQuery = referenceQueryBuilder.buildFilterQuery(
      assembler.convertQuery({ filter: opts?.filter }).filter,
    );
    const populated = await foundEntity.populate({
      path: relationName,
      match: filterQuery,
    });
  ...
  async queryRelations<Relation>(
    ...
    if (Array.isArray(dto)) {
      return dto.reduce(async (mapPromise, entity) => {
        const map = await mapPromise;
        const refs = await this.queryRelations(
          RelationClass,
          relationName,
          entity,
          query,
    ...
    const assembler = AssemblerFactory.getAssembler(RelationClass, Document);
    const { filterQuery, options } = referenceQueryBuilder.buildQuery(
  ...
  async addRelations<Relation extends Document>(
    ...
    if (relationIds.length !== refCount) {
      throw new Error(
        `Unable to find all ${relationName} to add to ${this.Model.modelName}`,
      );
    }
    await entity
      .updateOne({
  ...
  async setRelations<Relation extends Document>(
    ...
    if (relationIds.length !== refCount) {
      throw new Error(
        `Unable to find all ${relationName} to set on ${this.Model.modelName}`,
      );
    }
    await entity
      .updateOne({ [relationName]: relationIds } as UpdateQuery<Entity>)
      .exec();
    // reload the document
    return this.getById(id);
  }
  ...
  async setRelation<Relation extends Document>(
    ...
    if (refCount !== 1) {
      throw new Error(
        `Unable to find ${relationName} to set on ${this.Model.modelName}`,
      );
    }
    await entity
      .updateOne({ [relationName]: relationId } as UpdateQuery<Entity>)
      .exec();
    // reload the document
    return this.getById(id);
  }
  ...
  async removeRelation<Relation extends Document>(
    ...
    if (refCount !== 1) {
      throw new Error(
        `Unable to find ${relationName} to remove from ${this.Model.modelName}`,
      );
    }
    await entity
      .updateOne({
  ...
  async removeRelations<Relation extends Document>(
    ...
    if (relationIds.length !== refCount) {
      throw new Error(
        `Unable to find all ${relationName} to remove from ${this.Model.modelName}`,
      );
    }
    if (this.isVirtualPath(relationName)) {
      throw new Error(
        `RemoveRelations not supported for virtual relation ${relationName}`,
      );
    }
    await entity
      .updateOne({
  ...
  private checkForReference(
    ...
    if (this.isVirtualPath(refName)) {
      ...
      throw new Error(
        `${operation} not supported for virtual relation ${refName}`,
      );
    }
    throw new Error(
      `Unable to find reference ${refName} on ${this.Model.modelName}`,
    );
  }
  ...
  private getReferenceQueryBuilder<Ref extends Document>(
    refName: string,
  ): FilterQueryBuilder<Ref> {
    return new FilterQueryBuilder<Ref>(this.getReferenceModel(refName));
  }

  private getReferenceModel<Ref extends Document>(
    refName: string,
  ): MongooseModel<Ref> {
    const { db } = this.Model;
    if (this.isReferencePath(refName)) {
  ...
  private getRefCount<Relation extends Document>(
    ...
    const referenceQueryBuilder =
      this.getReferenceQueryBuilder<Relation>(relationName);
    return referenceModel
      .countDocuments(
        referenceQueryBuilder.buildIdFilterQuery(relationIds, filter),
      )
      .exec();
  }
}
